...
所有其他標點候選都使用數字鍵原位替換，例如按 `2` 可改用第二候選。繼續輸入
文字、Space 或 Return 只會結束標點候選狀態並照常處理該按鍵；若要緊接著
輸入數字，需先按 `Esc` 結束候選狀態。
...
符號亦提供左右長箭頭。
標點候選直接使用數字鍵選擇，例如 `$` 後按 `2` 選 `¥`，`.` 後按 `2` 選
`。`，`-` 後按 `2` 選 `－`。若要輸入 `$123`、`1.23`、`123-456` 或
`abc=123`，需在標點後先按 `Esc` 結束候選狀態，再輸入數字。
