...
| `Space` | 第一候選有 `◆` 時提交並繼續聯想；否則關閉聯想並交回應用程式 |
| `Esc` | 關閉聯想並清除上下文 |
...
標點狀態下，數字鍵只有在對應現有候選時才會被視為候選快捷鍵。例如半形
`-` 只有兩個候選，後續輸入 `4` 會先提交 `-`，再讓 `4` 以普通數字輸入，
因此 `123-456` 不需要在 `-` 後額外按 Space。

`$` 及 `.` 是例外：雖然它們有多個候選，`shouldContinueNumericPunctuationInput`
會讓其後的數字優先視為連續數字輸入，先提交符號再處理數字，因此 `$123`
及 `1.23` 可直接連續輸入。`Shift + 數字` 會略過此數字輸入特例，回到
候選快捷鍵，例如 `$` 後按 `Shift + 2` 會提交 `¥`，`.` 後按
`Shift + 2` 會提交 `。`。候選窗會以 `punctuationDisplayText` 顯示
`Shift：1 ...` 提示，避免與普通數字輸入混淆。
