...
sender 同時符合 `NSTextInputClient`，否則無法建立待替換範圍，數字選字鍵
便只會關閉候選而不會更改已輸出的標點。
...
| 按鍵 | 行為 |
| --- | --- |
| 繼續輸入文字或另一標點 | 關閉舊候選，再處理新按鍵 |
| `Return` 或 `Space` | 關閉候選，並讓目標 app 照常處理按鍵 |
| `1` 至 `0` | 將已輸出的標點原位替換成第一至第十個候選 |
| `Shift + 數字` | 不作候選選取，照常處理對應的標點或符號按鍵 |
| `Delete` | 關閉候選，並讓目標 app 刪除已輸出的標點 |
| `Esc` | 關閉候選，保留已輸出的標點 |

標點狀態下，普通數字固定作為候選快捷鍵。例如 `$` 後按 `2` 會提交 `¥`，
`.` 後按 `2` 會提交 `。`，`-` 後按 `2` 會提交 `－`。候選窗使用一般
`candidateText` 顯示 `1 ...` 至 `0 ...`，不再顯示 `Shift：` 提示。若要輸入
`$123`、`1.23`、`123-456` 或 `abc=123`，需在標點後先按 `Esc` 關閉候選，
再輸入數字。
