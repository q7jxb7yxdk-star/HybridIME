...
macOS 輸入 `vnd` 時會立即在宿主應用程式顯示該字母，候選區顯示 `1 好`；按 `1` 會把已輸入字母替換成「好」。`Return` 不會選取候選，而會交回宿主執行換行、送出或前往網址等原有動作。iOS 同樣會先透過 `UITextDocumentProxy` 把鍵入字母插入宿主應用程式，點選候選後再刪除該字碼並插入候選。
