...
字根鍵、數字鍵與符號鍵以鍵盤根視圖承載的非互動 overlay 顯示按住時的放大鍵帽，避免被列 stack view 裁切；觸控放開、取消、滑出或鍵盤重建時會隱藏。字根鍵帽同時顯示倉頡碼與英文字母：直向上下排列，iPhone 橫向左右排列並縮小為同一字級。頁面切換、Shift、Delete、Space、地球鍵與 Return 不顯示放大鍵帽。

在沒有自訂地球鍵的 iPhone 緊湊版，底列使用 `123／ABC`、`,`、Space、`.`、Return 的左右約束，而非 stack view；`123／ABC`、`,`、`.` 和 Return 共用頁面鍵寬度，Space 填滿其間可用空間。需要自訂地球鍵時保留原 stack view fallback，避免窄螢幕重疊。Return 不再依宿主的 `returnKeyType` 切換「完成／搜尋／前往」等文字，iPhone 與 iPad 均固定使用公開 SF Symbol `arrow.turn.down.left`；這是根據系統鍵盤截圖選擇的近似圖示，非可取用的私有鍵帽 asset。
...
候選列使用水平 `UIScrollView`。iOS 26 的捲動邊緣效果可能模糊這條只有 38 點高的列內文字，因此控制器會隱藏該捲動視圖四邊的效果，同時保留水平捲動。這項調整已通過原始碼與差異檢查，實際顯示效果仍待使用者在裝置上驗證。
...
長按 Space 會進入游標移動模式，但不會以覆蓋層遮住鍵盤。模式期間保留每顆鍵帽輪廓，隱藏按鍵文字、倉頡字根、替代符號及圖示，並根據系統外觀將鍵帽切換為 Light 淺灰或 Dark 深灰；候選列維持可見。放開或取消手勢、重建鍵盤或 extension 消失時會復原原有外觀。水平距離除以 10 點閾值後傳給 `adjustTextPosition`。垂直移動首先使用實際換行上下文與偏好的欄位；看不到邏輯換行時，會估算每行十個字元。此備援機制刻意採用近似值。
...
| --- | --- | --- | --- | --- |
| `HybridIME` | macOS 應用程式／輸入法 | `com.sunny.inputmethod.hybridime` | macOS 26.0 | 1.4.0（組建編號 20260913） |
| `HybridIMEiOS` | iOS／iPadOS 應用程式 | `com.sunny.inputmethod.hybridime.ios` | iOS 26.0 | 1.4.0（組建編號 20260913） |
| `HybridIMEKeyboard` | 鍵盤延伸功能 | `com.sunny.inputmethod.hybridime.ios.keyboard` | iOS 26.0 | 1.4.0（組建編號 20260913） |
...
發行腳本解析 `HybridIME` macOS scheme 的 Debug 與 Release 建置設定，除非兩者都使用版本 `1.4.0`、組建編號 `20260913`、部署目標 `26.0`，以及預期的 macOS 建置目標與套件識別碼，否則會硬停止。此預檢不會執行或證明簽署、公證、封裝釘選或發佈。
...
- 當解析後 macOS `HybridIME` scheme 的 Debug 或 Release 設定偏離版本 `1.4.0`、組建編號 `20260913`、部署目標 `26.0`、預期建置目標或套件識別碼時，`Scripts/release.sh` 會硬停止。
