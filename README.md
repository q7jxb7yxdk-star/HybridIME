...
- 聽寫等系統按鍵會在組字或候選狀態下釋放輸入法狀態，讓 macOS 接管。
- 提交中文或英文後顯示同語言的聯想候選。
- 聯想候選會按本機使用次數逐步調整排序。
...
- 若目標 app 不提供游標前文字，HybridIME 會優先使用中文標點；直接鍵入數字後則仍使用半形標點。

預設標點會立即顯示，不需要按 `Space` 或 `Return`。繼續輸入文字或數字時
會自動確認第一候選；例如可連續輸入 `123-456`。按 `2` 可改用另一種形式，
按 `Delete` 或 `Esc` 可取消尚未確認的標點。
...
美元、小於及大於符號的次序不按語境調整。`` ` ``、`-`、`+`、`=` 任何
語境均預設半形。較少使用的全形 `＄` 固定排在美元候選最後，小於及大於
符號亦提供左右長箭頭。
輸入 `$` 或 `.` 後可直接接數字，例如 `$123`、`1.23`，不需要先按 `Space`
確認符號。若要在這兩個符號狀態下選擇其他候選，可使用 `Shift + 數字`，
例如 `$` 後 `Shift + 2` 選 `¥`，`.` 後 `Shift + 2` 選 `。`。
候選窗會顯示 `Shift：1 .  2 。  3 ⋯⋯` 或
`Shift：1 $  2 ¥  3 £  4 €  5 ₹  6 ₺  7 ＄` 作提示。

容易混淆的半形/全形符號會在候選窗加上標籤，但實際輸出仍然只是符號：
...
HybridIME 會在本機記錄每組輸入碼實際選擇的中文字候選。同一字碼選擇
中文字候選一次後，該候選便會成為智能預測，並在候選視窗中以 `◆` 標示。

第一候選若是已標示的中文預測候選，按 Space 會直接提交該候選。這項規則不計算百分比，
也不依賴前文；只根據「同一字碼 + 同一中文字候選」的選擇記錄。若同一
字碼選過多個中文字候選，會優先使用選擇次數較多、最近選過的候選。

原始英文、英文字典翻譯、標點及聯想候選不參與這套智能學習。偵測到
英文句段時，中文智能預測亦不會套用，避免誤選中文字一次後令 `you`、
`Step` 這類英文輸入被自動取代。
...
- 按 `Return` 或數字鍵選擇聯想。
- 最近選過的聯想會在相同情境排第一並顯示 `◆`；此時按 `Space` 可直接
  提交第一個聯想字並繼續聯想。
- 按 `Esc` 關閉聯想。
