...
- 倉頡五代單字解碼，輸入一至五碼時列出完整碼以目前輸入開頭的單字候選；解碼器以明確迴圈依序去重，避免 Release 最佳化下遺失 prefix 結果。超過五碼仍可輸入英文。
...
- 沒有組字或候選時，`=` 的原始按鍵事件會交回宿主應用程式，讓 Google Sheets 等依賴按鍵語義的介面可直接進入公式模式；組字期間仍保留 `=`／`＝` 標點候選。
...
- `123` 頁面的單引號與雙引號鍵帽分別顯示 `‘`、`“`；iPad 寬版保留向下輕掃輸入 `{`、`}`。
...
- `Scripts/release.sh` 的前置檢查會讀取 `HybridIME` macOS scheme 的 Debug／Release 已解析建置設定，並硬性要求版本 `1.4.0`、組建編號 `20260913` 與部署目標 `26.0`。這只準備發佈流程，並不代表簽署、公證或發佈已完成。
