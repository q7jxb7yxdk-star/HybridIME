...
final class UserLearningStore: @unchecked Sendable {
    static let shared = UserLearningStore()
    ...
    private func openDatabase(at url: URL?) {
        guard let url else {
            NSLog("HybridIME could not create the user learning directory")
            return
        }
        let flags = SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_FULLMUTEX
        guard sqlite3_open_v2(url.path, &database, flags, nil) == SQLITE_OK else {
            NSLog("HybridIME could not open the user learning database")
            closeDatabase()
            return
        }
    ...
    func recordAssociationSelection(
        ...
        lock.withLock {
            ...
            let increment = incrementingCount ? 1 : 0
            execute(
                """
    ...
    nonisolated private static func databaseURL() -> URL? {
        ...
        do {
            ...
            NSLog("HybridIME could not prepare Application Support: \(error)")
            return nil
        }
    }
}
