...
- 按 `Space`：一般輸出英文及一個空格；已學習的中文或原始英文候選可自動選取。
...
第一候選若是已標示的智能預測候選，按 Space 會直接提交該候選。這項規則不計算百分比，
也不依賴前文；只根據「同一字碼 + 同一候選」的選擇記錄。若同一字碼選過
多個中文字候選或原始英文，最後一次明確選取的候選會立即成為首選；不會累計或比較
選取次數。原始英文會保留大小寫，只有再次輸入完全相同大小寫的 buffer 時才會成為
首選；此時按 `Space` 會提交原始英文且不加空格。

英文字典翻譯、標點及聯想候選不參與這套智能學習。偵測到英文句段時，
中文智能預測不會套用，避免誤選中文字一次後令 `you`、`Step` 這類英文輸入被
自動取代；已學習且大小寫相同的原始英文仍可成為首選。
