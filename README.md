...
- iPad 在水平尺寸類別為 `regular` 且鍵盤寬度至少 700 點時使用 11 格寬版配置；依同一裝置的原生倉頡截圖量度，縱向高度為 340 點、橫向高度為 428 點。直向或非寬版 iPhone 維持 260 點緊湊配置。
- iPhone 橫向時沿用緊湊版的 QWERTY、Shift、Delete 與底列位置，按鍵隨可用寬度成為橫向比例；字母鍵以左側倉頡字根、右側英文字母顯示。此模式使用按同裝置原生倉頡截圖量度的 180 點自訂內容高度，內部列與按鍵盡量填滿可用高度。Shift／Delete、頁面鍵及 Return 依方向使用從原生倉頡截圖量度的寬度。
...
- iPhone 橫向使用 180 點自訂內容高度；此值來自單一裝置的 Apple 內建倉頡截圖量度，無公開 API 可讀取或保證在其他裝置及宿主 App 同步原生高度，其按鍵比例、字根／英文標籤與地球鍵仍需實機驗證。
